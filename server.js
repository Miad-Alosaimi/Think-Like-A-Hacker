...
// Credentials are stored as a list so multiple admin accounts can exist side
// by side. Older installs that still have the single-object format are
// transparently upgraded to a list the first time they're read.
...
function saveCredentials(credentialsList) {
    try {
        fs.writeFileSync(credentialsFile, JSON.stringify(credentialsList, null, 2));
        return true;
    } catch (error) {
...
function findAdminByUsername(username) {
    return loadCredentials().find(c => c.username === username) || null;
}
...
// Create an admin account. Works both for first-time setup and for adding
// additional admins later - the only requirement is a unique username.
app.post('/api/admin/setup', async (req, res) => {
    try {
        ...
        const credentialsList = loadCredentials();
        if (credentialsList.some(c => c.username === username)) {
            return res.json({
                success: false,
                message: 'An admin account with that username already exists'
            });
        }
        ...
        if (saveCredentials(credentialsList)) {
...
app.get('/admin/setup', (req, res) => {
    // Always reachable - lets an admin create additional accounts later,
    // not just during the very first install.
...
server.on('error', (error) => {
    if (error.code === 'EADDRINUSE') {
        ...
        console.error(`Another instance of this server (or another app) is already running on port ${PORT}.`);
        console.error(`Stop that process first, or set a different PORT, then try again.\n`);
    } else {
